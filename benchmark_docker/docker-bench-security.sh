...
if ! docker ps -q > /dev/null 2>&1; then
  printf "Error connecting to docker daemon (does docker ps work?). Try systemctl status docker\n"
  exit 1
fi
...
usage () {
  ...
  -v VESION    optional  Current Docker runtime official version. By default takes the current day.
EOF
}
...
while getopts bhl:c:e:i:x:t:n:v: args
do
  case $args in
  b) nocolor="nocolor";;
  h) usage; exit 0 ;;
  ...
  x) exclude="$OPTARG" ;;
  n) limit="$OPTARG" ;;
  v) docker_current_version="$OPTARG" ;;
  *) usage; exit 1 ;;
  esac
done
...
main () {
  ...
  printf "\n"
  note "Checks: $totalChecks"
  # TODO: show warn/info/note depending of $currentScore
  note "Score: $currentScore"
