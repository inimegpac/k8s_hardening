...
check_5_1() {
  ...
  for c in $containers; do
    ...
    if [ "$policy" = "AppArmorProfile=" ] || [ "$policy" = "AppArmorProfile=[]" ] || [ "$policy" = "AppArmorProfile=<no value>" ] || [ "$policy" = "AppArmorProfile=unconfined" ]; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_1"
        note "     * No AppArmorProfile Found: $c"
...
check_5_2() {
  ...
  for c in $containers; do
    ...
    if [ "$policy" = "SecurityOpt=" ] || [ "$policy" = "SecurityOpt=[]" ] || [ "$policy" = "SecurityOpt=<no value>" ]; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_2"
        note "     * No SecurityOptions Found: $c"
...
check_5_3() {
  ...
  for c in $containers; do
    ...
    if [ "$caps" != 'CapAdd=' ] && [ "$caps" != 'CapAdd=[]' ] && [ "$caps" != 'CapAdd=<no value>' ] && [ "$caps" != 'CapAdd=<nil>' ]; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_3"
        note "     * Capabilities added: $caps to $c"
...
check_5_4() {
  ...
  for c in $containers; do
    ...
    if [ "$privileged" = "true" ]; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_4"
        note "     * Container running in Privileged mode: $c"
  ...
  for c in $containers; do
    ...
    for v in $sensitive_dirs; do
      ...
      if [ $sensitive -eq 1 ]; then
        ...
        if [ $fail -eq 0 ]; then
          warn "$check_5_5"
          note "     * Sensitive directory $v mounted in: $c"
...
check_5_7() {
  ...
  for c in $containers; do
    ...
    if [ -n "$port" ] && [ "$port" -lt 1024 ]; then
        ...
        if [ $fail -eq 0 ]; then
          warn "$check_5_7"
          note "     * Privileged Port in use: $port in $c"
...
check_5_9() {
  ...
  for c in $containers; do
    ...
    if [ "$mode" = "NetworkMode=host" ]; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_9"
        note "     * Container running with networking mode 'host': $c"
...
check_5_10() {
  ...
  for c in $containers; do
    ...
    if [ "$memory" = "0" ]; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_10"
        note "     * Container running without memory restrictions: $c"
...
check_5_11() {
  ...
  for c in $containers; do
    ...
    if [ "$shares" = "0" ]; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_11"
        note "     * Container running without CPU restrictions: $c"
  ...
  for c in $containers; do
   read_status=$(docker inspect --format '{{ .HostConfig.ReadonlyRootfs }}' "$c")
    ...
    if [ "$read_status" = "false" ]; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_12"
        note "     * Container running with root FS mounted R/W: $c"
  ...
  for c in $containers; do
    for ip in $(docker port "$c" | awk '{print $3}' | cut -d ':' -f1); do
      if [ "$ip" = "0.0.0.0" ]; then
        ...
        if [ $fail -eq 0 ]; then
          warn "$check_5_13"
          note "     * Port being bound to wildcard IP: $ip in $c"
...
check_5_14() {
  ...
  for c in $containers; do
    ...
    if [ "$policy" != "MaximumRetryCount=5" ]; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_14"
        note "     * MaximumRetryCount is not set to 5: $c"
...
check_5_15() {
  ...
  for c in $containers; do
    ...
    if [ "$mode" = "PidMode=host" ]; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_15"
        note "     * Host PID namespace being shared with: $c - NOT run with --pid=host argument"
...
check_5_16() {
  ...
  for c in $containers; do
    ...
    if [ "$mode" = "IpcMode=host" ]; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_16"
        note "     * Host IPC namespace being shared with: $c - NOT run with --ipc=host argument"
...
check_5_18() {
  ...
  for c in $containers; do
    ...
    if [ "$ulimits" = "Ulimits=" ] || [ "$ulimits" = "Ulimits=[]" ] || [ "$ulimits" = "Ulimits=<no value>" ]; then
      ...
      if [ $fail -eq 0 ]; then
        info "$check_5_18"
        note "     * Container no default ulimit override: $c - Override if needed"
        no_ulimit_containers="$no_ulimit_containers $c"
        fail=1
      else
        info "     * Container no default ulimit override: $c - Override if needed"
        no_ulimit_containers="$no_ulimit_containers $c"
      fi
    fi
  done
  ...
  for c in $containers; do
    if docker inspect --format 'Propagation={{range $mnt := .Mounts}} {{json $mnt.Propagation}} {{end}}' "$c" | \
     grep shared 2>/dev/null 1>&2; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_19"
        note "     * Mount propagation mode is shared: $c"
...
check_5_20() {
  ...
  for c in $containers; do
    ...
    if [ "$mode" = "UTSMode=host" ]; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_20"
        note "     * Host UTS namespace being shared with: $c - NOT run with --uts=host argument"
...
check_5_21() {
  ...
  id_5_21="5.21"
  desc_5_21="Ensure the default seccomp profile is not Disabled (Scored)"
  ...
  for c in $containers; do
    if docker inspect --format 'SecurityOpt={{.HostConfig.SecurityOpt }}' "$c" | \
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_21"
        note "     * Default seccomp profile disabled: $c"
...
check_5_22() {
  ...
  totalChecks=$((totalChecks + 1))
  info "$check_5_22"
  note "     * NOT use the --privileged option in docker exec command"
  note "     * You can audit with section 1 and the following command"
  note "     * ausearch -k docker | grep exec | grep privileged"
  resulttestjson "NOTE"
  currentScore=$((currentScore + 0))
}
...
check_5_23() {
  ...
  totalChecks=$((totalChecks + 1))
  info "$check_5_23"
  note "     * NOT use the --user=root option in docker exec command"
  note "     * You can audit with section 1 and the following command"
  note "     * ausearch -k docker | grep exec | grep user"
  resulttestjson "NOTE"
  currentScore=$((currentScore + 0))
}
...
check_5_24() {
  ...
  for c in $containers; do
    ...
    if [ "$mode" != "CgroupParent=x" ]; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_24"
        note "     * Confirm cgroup usage: $c - NOT run with --cgroup-parent argument"
...
check_5_25() {
  ...
  for c in $containers; do
    if ! docker inspect --format 'SecurityOpt={{.HostConfig.SecurityOpt }}' "$c" | grep 'no-new-privileges' 2>/dev/null 1>&2; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_25"
        note "     * Privileges not restricted: $c - RUN with --security-opt=no-new-privileges argument"
...
check_5_26() {
  ...
  for c in $containers; do
    if ! docker inspect --format '{{ .Id }}: Health={{ .State.Health.Status }}' "$c" 2>/dev/null 1>&2; then
      if [ $fail -eq 0 ]; then
        warn "$check_5_26"
        note "     * Health check not set: $c - RUN with --health-cmd argument"
...
check_5_28() {
  ...
  id_5_28="5.28"
  desc_5_28="Ensure that the PIDs cgroup limit is used (Scored) (Docker Compose deprecated)"
  ...
  for c in $containers; do
    ...
    if [ "$pidslimit" = "0" ] || [  "$pidslimit" = "<nil>" ] || [  "$pidslimit" = "-1" ]; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_28"
        note "     * PIDs limit not set: $c - RUN with --pids-limit argument"
      ...
      docker0Containers=$(docker network inspect --format='{{ range $k, $v := .Containers }} {{ $k }} {{ end }}' "$net" | \
        ...
        if [ -n "$docker0Containers" ]; then
          ...
          for c in $docker0Containers; do
            ...
            if [ -n "$cName" ]; then
              info "     * Container in docker0 network: $cName - Create/link a network for this container"
...
check_5_30() {
  ...
  for c in $containers; do
    if docker inspect --format '{{ .HostConfig.UsernsMode }}' "$c" 2>/dev/null | grep -i 'host' >/dev/null 2>&1; then
      ...
      if [ $fail -eq 0 ]; then
        warn "$check_5_30"
        note "     * Namespace shared: $c - NOT run with --userns=host argument"
